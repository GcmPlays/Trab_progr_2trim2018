...
int separacor(FILE *adr, struct bmpheader *ptrheader,struct bmpinfoheader *ptrinfo, char *arquivo)
{
    ...
    for(i = 0; i < altura; i++)
    {
        for(j = 0; j < largura; j++)
        {
            ...
            if(cor != white)
            {
            if(option==red)	cor-=cor%65536;
			if(option==blue) cor=cor%256;
			if(option==green) cor=cor%65536-cor%256;
...
int grayscale(char *arquivo,char *arquivo2)
	{
    ...
    unsigned char cor[3];
    FILE *open;
    open=fopen(arquivo,"rb"); //abre arquivo
    ...
    strcat(arquivo2,"_gs.bmp");
	open=fopen(arquivo2,"wb");
